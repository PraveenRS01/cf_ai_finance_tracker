...
// .wrangler/tmp/bundle-wOvLq5/strip-cf-connecting-ip-header.js
...
var FinancialAgent = class {
  ...
  async getFinancialSummary() {
    ...
    const monthlyExpenses = expenses.filter((exp) => new Date(exp.date) >= thirtyDaysAgo).reduce((sum, exp) => sum + exp.amount, 0);
    const upcomingBills = bills.filter((bill) => new Date(bill.due_date) >= /* @__PURE__ */ new Date()).reduce((sum, bill) => sum + bill.amount, 0);
  ...
  async parseAddBill(message) {
    ...
    const dueDate = this.extractDate(message) || new Date(Date.now() + 30 * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
  ...
  async parseSetSavingsGoal(message) {
    ...
    const targetDate = this.extractDate(message) || new Date(Date.now() + 365 * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
    const monthlyContribution = Math.ceil(targetAmount / 12);
...
__name(FinancialAgent, "FinancialAgent");
...
var src_default = {
  async fetch(request, env2, ctx) {
    ...
    if (url.pathname === "/chat" || url.pathname === "/api/financial-data") {
      const id = env2.FinancialAgent.idFromName("financial-agent");
      const obj = env2.FinancialAgent.get(id);
...
// .wrangler/tmp/bundle-wOvLq5/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-wOvLq5/middleware-loader.entry.ts
